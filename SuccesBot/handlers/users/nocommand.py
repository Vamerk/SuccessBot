...
async def dd_message(message: types.Message):
    ...
    if 'Повысить уровень' in message.text:
        pers = Person(message.from_user.id)
    ...
    if 'Открыть лутбокс' in message.text:
        if int(Inventory(message.from_user.id).lootboxs_s()) >= 1:
            mi = await bot.send_animation(message.chat.id, animation=open(f'Image/open_lootbox.gif.mp4', 'rb'))
            time.sleep(2)
            await bot.delete_message(message.chat.id, message_id=mi.message_id)
    ...
    if 'Открыть премиум лутбокс'  in message.text:
